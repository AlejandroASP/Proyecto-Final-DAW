...
const Product = ({ id, name, price, addToCart }) => (
  <div className="p-4 mb-4 mx-4 rounded bg-white bg-opacity-75 md:flex justify-between md:h-25">
    <div className='md:flex items-center'>
      <img src={ProductoBeta} alt="helldivers" className='w-full md:w-1/3 rounded-xl'/>
      <h2 className='md:ml-4'>{name}</h2>
    </div>
    <div className='flex flex-col'>
      <p>Precio: ${price}</p>
      <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold mt-3 py-2 px-4 rounded" onClick={() => addToCart({ id, name, price })}>Agregar al carrito</button>
    </div>
  </div>
);
...
const RecommendProduct = ({ id, name, price, addToCart }) => (
  <div className="p-4 mb-4 mx-4 rounded bg-white bg-opacity-75 md:flex flex-row justify-baseline md:h-25">
    ...
    <div className='flex flex-col items-baseline'>
      ...
      <p>Precio: ${price}</p>
      <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold md:mr-4 mt-3 py-2 px-4 rounded" onClick={() => addToCart({ id, name, price })}>Agregar al carrito</button>
    </div>
  </div>
);
...
const CartItem = ({ id, name, price, removeFromCart }) => (
  <div className="border p-4 mb-4 bg-white bg-opacity-75">
    <div className='flex flex-col'>
      ...
      <button className="bg-red-500 hover:bg-red-700 text-white font-bold mt-3 py-2 px-4 rounded" onClick={() => removeFromCart(id)}>Eliminar</button>
    </div>
  </div>
);
...
const Cart = ({ cart, removeFromCart, clearCart, completePurchase }) => (
  <div className="bg-black bg-opacity-45 p-4 mb-4 md:w-2/6 rounded-xl border border-white-500 border-4 mt-4 md:mt-0">
    <h1 className='text-white text-2xl font-extrabold mb-4'>Carrito de Compras</h1>
    {cart.length === 0 ? (
      <div>
        ...
        <>
          ...
          <h2 className='text-white'>Total: ${cart.reduce((acc, item) => acc + item.price, 0)}</h2>
        ...
        <div className="container mx-auto p-5">
          ...
          <div className='flex flex-col md:flex-row justify-between'>
            <div className="md:w-4/5 bg-black bg-opacity-45 md:mr-7 rounded-xl border border-white-500 border-4 pb-4">
              <div className='h-15'>
                <h1 className='text-white text-2xl font-extrabold m-4'>Productos seleccionados</h1>
              </div>
            ...
            {purchaseSuccess ? (
              <p className="bg-black bg-opacity-45 md:w-2/6 rounded-xl mt-4 border border-white-500 border-4 text-white text-center p-4">Compra realizada con éxito</p>
            ) : (
              <Cart
                cart={cart}
          ...
          <div className='bg-black bg-opacity-45 mt-7 rounded-xl border border-white-500 border-4 pb-4'>
            <h1 className='text-white text-2xl font-extrabold m-4'>Productos Relacionados</h1>
              <div className="grid grid-cols-1 gap-1 md:grid-cols-3 overflow-auto md:overflow-none h-96 md:h-40">
                <RecommendProduct id={4} name="Producto 4" price={40} addToCart={addToCart} />
                <RecommendProduct id={5} name="Producto 5" price={50} addToCart={addToCart} />
                <RecommendProduct id={6} name="Producto 6" price={60} addToCart={addToCart} />
              </div>
          </div> 
        </div>
      </div>
